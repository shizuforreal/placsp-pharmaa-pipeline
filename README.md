...
I also widened the link-detection logic in `search.py` once I noticed PLACSP renders some result rows as a `deeplink:detalle_licitacion` query-string URL and others as a WebSpherePortalstyle `!ut/p/z1/...` path-encoded URL — visually identical on the results page, but only one of those formats was being picked up. Both share the substring `detalle`, so matching is now scoped to that instead of one specific URL shape.
